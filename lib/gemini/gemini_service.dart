import 'dart:convert';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:google_generative_ai/google_generative_ai.dart';

String buildTravelPrompt({
  required String region,
  required String subRegion,
  required List<String> themes,
  required String transport,
  required String date,
}) {
  final themeList = themes.join(', ');
  return '''
한국 $region $subRegion에서 여행 일정을 짜줘.
테마는 "$themeList"이고, 교통수단은 "$transport"야.
오전 9시부터 오후 6시까지 하루 일정으로,
장소 이름, 간단한 설명(간단한 한 문장, ~한 시간/~한 식사/~한 여행 같은 형식으로), 위도와 경도, 시작 시간과 종료 시간 형식으로 알려줘.
총 5개의 일정을 제안해 줘.
응답은 반드시 JSON 배열 형식으로 주고, 각 객체는 다음 필드를 포함해야 해:

- travel_title
- description
- place_name
- lat
- lng
- start (예: ${date}T09:00:00)
- end (예: ${date}T10:30:00)

응답은 반드시 JSON 배열 형식으로 주되, ``` 같은 마크다운 형식은 절대 포함하지 마.
''';
}

Future<List<Map<String, dynamic>>> fetchGeminiSchedule({
  required String roomId,
  required String region,
  required String subRegion,
  required List<String> themes,
  required String transport,
  required String date,
  required String apiKey,  // 👉 Gemini API 키
}) async {
  final prompt = buildTravelPrompt(
    region: region,
    subRegion: subRegion,
    themes: themes,
    transport: transport,
    date: date,
  );


  final model = GenerativeModel(model: 'gemini-1.5-flash', apiKey: apiKey);
  final chat = model.startChat();

  final response = await chat.sendMessage(Content.text(prompt));

  final content = response.text;
  if (content == null) throw Exception("응답이 비어 있음 ❌");

  try {
    // 마크다운 코드블럭 제거 처리
    final cleaned = content
        .replaceAll(RegExp(r'^```(?:json)?\s*', multiLine: true), '')
        .replaceAll(RegExp(r'\s*```$', multiLine: true), '');

    final parsed = jsonDecode(cleaned);
    final schedule = List<Map<String, dynamic>>.from(parsed);

    // 🔸 Firestore에 저장
    final firestore = FirebaseFirestore.instance;
    final scheduleCollection = firestore.collection('travel_rooms').doc(roomId).collection('schedules');
    final batch = firestore.batch();

    for (final item in schedule) {
      final docRef = scheduleCollection.doc(); // 자동 ID
      batch.set(docRef, {
        ...item,
        'created_at': Timestamp.now(),
      });
    }

    await batch.commit();
    print('✅ 일정 생성 및 저장 완료!');

    return schedule;
  } catch (e) {
    throw Exception("JSON 파싱 실패 ❌\n응답 내용: $content");
  }
}